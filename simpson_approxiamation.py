...
if __name__ == '__main__':
    ...
    # Calcul des prix des options asiatiques pour chaque valeur de volatilité
    option_prices = [asian_option_price_put(S0, r, K, T, N, sigma, Nmc) for sigma in sigmas]
    ...
    for i, S in enumerate(S_values):
        for j, sigma in enumerate(sigmas):
            option_prices[i, j] = asian_option_price_put(S, r, K, T, N, sigma, Nmc)
...
def asian_option_price_call_flottant(S0, r, K, T, N, sigma, Nmc):
    ...
    S_avg = np.zeros(Nmc)
    payoffs = np.zeros(Nmc)
    integral_approximation = np.zeros(Nmc)

    for i in range(Nmc):
        W = np.random.randn(1) * np.sqrt(dt)
        integral_approximation[i] = simpson_rule(lambda t: geometric_brownian_motion(t, S0, r, sigma, W), 0, T,
                                              N)
        S_avg[i] = np.mean(integral_approximation)
        payoffs[i] = np.maximum(integral_approximation[Nmc - 1] - S_avg[i], 0)
...
if __name__ == '__main__':
    ...
    # Calcul des prix des options asiatiques pour chaque valeur de volatilité
    option_prices = [asian_option_price_call_flottant(S0, r, K, T, N, sigma, Nmc) for sigma in sigmas]
    ...
    for i, S in enumerate(S_values):
        for j, sigma in enumerate(sigmas):
            option_prices[i, j] = asian_option_price_call_flottant(S, r, K, T, N, sigma, Nmc)
...
def asian_option_price_put_flottant(S0, r, K, T, N, sigma, Nmc):
    dt = T / N
    S_avg = np.zeros(Nmc)
    payoffs = np.zeros(Nmc)
    integral_approximation = np.zeros(Nmc)
    ...
    for i in range(Nmc):
        ...
        integral_approximation[i] = simpson_rule(lambda t: geometric_brownian_motion(t, S0, r, sigma, W), 0, T,
                                              N)
        S_avg[i] = np.mean(integral_approximation)
        payoffs[i] = np.maximum(S_avg[i] - integral_approximation[Nmc - 1], 0)
...
if __name__ == '__main__':
    ...
    # Calcul des prix des options asiatiques pour chaque valeur de volatilité
    option_prices = [asian_option_price_put_flottant(S0, r, K, T, N, sigma, Nmc) for sigma in sigmas]
    ...
    for i, S in enumerate(S_values):
        for j, sigma in enumerate(sigmas):
            option_prices[i, j] = asian_option_price_put_flottant(S, r, K, T, N, sigma, Nmc)
